...
L1=Label(window, text = 'Add new job offer ',font="none 25 italic ",bg='#000044',fg='#ffffff')
...
def add() :
    f=open('job.csv', 'a',newline='')
    code=zone.get()
    with  open('job.csv', 'r') as f1:
      read=csv.reader(f1)
      for line in read:
        while line[0] == code:
    ...
    mes_desc=zone9.get()
    msg=[code,name ,address,phone,email,degree, exprience ,qualification,mes_desc]
    if (code=="") or (name=="") or (address=="") or (email=="") or (phone=="") or (degree=="") or (qualification=="") or (exprience=="") or (mes_desc=="") :
      tkinter.messagebox.showinfo("Fields error", " Fields are empty ", icon="error")
    else :
      writr=csv.writer(f)
      writr.writerow(msg)
      tkinter.messagebox.showinfo("", "job added successfully ", icon="info")
    f.close()
    
   
B = Button(window, text="add ",bd=5,command=add ).place(x=400,y=650,height=35,width=90) 
